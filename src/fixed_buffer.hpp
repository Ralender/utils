/*
 * type similar to std::vector but for non-copyable and non-movable types. with
...
template <typename T> struct fixed_buffer_base {
public:
  using value_type = T;
...
public:
  static constexpr bool is_noexcept_destructible =
...
private:
  base &_data() { return *static_cast<base *>(this); }
  static base init(iterator ptr) { return base{ptr, ptr}; }
  ...
  void create_at(iterator it, Ts &&... ts) noexcept(
      std::is_nothrow_constructible<value_type, Ts...>::value) {
    new (it) value_type(std::forward<Ts>(ts)...);
  }
  std::string out_range_msg(size_type pos, const std::string &position) {
    return ("out of range : " + std::to_string(pos) +
            " >= " + std::to_string(size()) + " in " + position);
  }
...
public:
  ...
  explicit fixed_buffer(size_type size)
      : base{init(
            static_cast<iterator>(std::malloc(sizeof(value_type) * size)))} {}
  template <typename C, typename... Ts>
  explicit fixed_buffer(size_type size, C &&c, Ts &&... ts)
      : fixed_buffer(size) {
    if (size > 0) {
  ...
  fixed_buffer &
  operator=(fixed_buffer &&other) noexcept(is_noexcept_destructible) {
    destroy();
    this->_data() = other._data();
    other._data() = {};
    return *this;
  ...
  reference operator[](size_type pos) noexcept { return _begin[pos]; }
  ...
  template <typename... Ts>
  reference remplace_at(size_type pos, Ts &&... ts) noexcept(
      std::is_nothrow_constructible<value_type, Ts...>::value
          &&is_noexcept_destructible) {
    destroy_at(_begin + pos);
    create_at(_begin + pos, std::forward<Ts>(ts)...);
    return _begin[pos];
  }
  template <typename... Ts>
  reference emplace(Ts &&... ts) noexcept(
      std::is_nothrow_constructible<value_type, Ts...>::value) {
    create_at(_end++, std::forward<Ts>(ts)...);
    return back();
  }
  ...
  void resize(size_type size) noexcept { _end = _begin + size; }
  ...
  ~fixed_buffer() noexcept(is_noexcept_destructible) { destroy(); }
};
