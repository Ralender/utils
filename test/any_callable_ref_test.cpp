...
TEST(callable_ref, call_stateless_lambda_int) {
  sg::any_callable_ref<int(int)> add1([](int i) { return i + 1; });
  ASSERT_EQ(add1(1), 2);
  ASSERT_EQ(add1(add1(1)), 3);
}

TEST(callable_ref, call_stateless_lambda_string) {
  sg::any_callable_ref<std::string(std::string)> add1(
...
TEST(callable_ref, call_stateless_lambda_int_ref) {
  sg::any_callable_ref<void(int &)> add1([](int &i) { i = i + 1; });
  int i = 1;
  add1(i);
  ASSERT_EQ(i, 2);
  add1(i);
...
TEST(callable_ref, call_stateless_lambda_string_ref) {
  sg::any_callable_ref<void(std::string &)> add1(
      [](std::string &i) { i += 'a'; });
  std::string tmp = "b";
  ASSERT_EQ(tmp, "b");
  add1(tmp);
...
TEST(callable_ref, call_statefull_ref_lambda_string) {
  std::string str;
  auto pinned_callback = [&]() { str.push_back('a'); };
