...
TEST(callable_ref, call_stateless_lambda_int) {
  sg::callable_ref<int(int)> add1([](int i){
    return i + 1;
  });
  ASSERT_EQ(add1(1), 2);
...
TEST(callable_ref, call_stateless_lambda_string) {
  sg::callable_ref<std::string(std::string)> add1([](std::string i){
    return i + 'a';
  });
  ASSERT_EQ(add1(""), "a");
...
TEST(callable_ref, call_stateless_lambda_string_ref) {
  sg::callable_ref<void(std::string&)> add1([](std::string& i){
    i += 'a';
  });
  std::string tmp = "b";
  ASSERT_EQ(tmp, "b");
...
TEST(callable_ref, call_statefull_ref_lambda_int) {
  ...
  sg::callable_ref<void()> add1(pinned_callback);
  ASSERT_EQ(i, 0);
  add1();
  ASSERT_EQ(i, 1);
  add1();
...
TEST(callable_ref, call_statefull_ref_lambda_string) {
  ...
  sg::callable_ref<void()> adda(pinned_callback);
  ASSERT_EQ(str, "");
  adda();
  ASSERT_EQ(str, "a");
  adda();
...
TEST(callable_ref, call_statefull_copy_lambda_int) {
  ...
  sg::callable_ref<int()> ref(pinned_callback);
  ASSERT_EQ(ref(), -2);
}
...
TEST(callable_ref, call_statefull_copy_lambda_string) {
  ...
  sg::callable_ref<std::string()> ref(pinned_callback);
  ASSERT_EQ(ref(), "a");
}

TEST(callable_ref, call_func_ptr_int) {
  sg::callable_ref<int(int)> ref(&free_add1);
  ASSERT_EQ(ref(1), 2);
  ASSERT_EQ(ref(ref(1)), 3);
}

TEST(callable_ref, call_func_ptr_int_ref) {
  sg::callable_ref<void(int&)> ref(&free_add1_ref);
  int tmp = 0;
  ASSERT_EQ(tmp, 0);
  ref(tmp);
  ASSERT_EQ(tmp, 1);
...
TEST(callable_ref, call_func_ptr_global_state) {
  global = 0;
  sg::callable_ref<void()> ref(free_func);
  ASSERT_EQ(global, 0);
  ref();
  ASSERT_EQ(global, 1);
  ref();
...
TEST(callable_ref, copy_call_stateless_lambda) {
  ...
  sg::callable_ref<int(int)> cop(add1);
  ASSERT_EQ(cop(1), 2);
...
TEST(callable_ref, copy_call_statefull_copy_lambda_string) {
  ...
  sg::callable_ref<std::string()> ref(pinned_callback);
  sg::callable_ref<std::string()> cop(ref);
  ASSERT_EQ(ref(), "a");
}
