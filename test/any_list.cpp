...
void for_tuple_impl(const std::tuple<Ts...> &tuple, F &&func,
...
void for_tuple(const std::tuple<Ts...> &tuple, F &&func) {
  for_tuple_impl(tuple, std::forward<F>(func),
                 std::make_index_sequence<sizeof...(Ts)>{});
}

template <typename...> using void_t = int;
...
TEST(any_list, basic) {
  ...
  for_tuple(t, [&](auto elem, auto) { list.push_back(elem); });
